...
**技术选型（审计意见采纳）：**
- 前端：**React + Vite**，构建产物通过 `go:embed` 嵌入 Go 二进制，保持单一二进制部署
- 不使用 Go html/template——书库管理 UI 需要网格/列表视图、封面缩略图、元数据编辑表单、搜索过滤器等复杂交互，服务端模板做这些体验差
- 竞品技术选型参考：Kavita=Angular、Audiobookshelf=Vue、Jellyfin=React、Immich=Svelte，无一使用服务端模板
- 设计参考：Kavita（现代感）+ Immich（简洁大气）+ Audiobookshelf（功能性）
- API 设计参考：**Audiobookshelf**（多端同步、API 文档、事件推送做得最好）
...
| 风险 | 级别 | 应对 |
|------|------|------|
| README 对外宣传与实际能力不符 | 🔴 高 | ✅ **已修正**，App/Server 分开列，未实现移到 Roadmap |
| Anx Reader 上游自己做服务端 | 🟡 中 | 保持差异化在 Server 端；定期观察上游动态 |
| 独立开发者精力有限 | 🟡 中 | AI 辅助下总工期 5-8 周可完成；瓶颈在设计决策和 UX 打磨，非编码速度 |
