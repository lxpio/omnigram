...
## 九、STT 反向验证结果（macOS ARM + iOS Simulator）

> 验证日期：2026-03-22
> 方法：sherpa-onnx Python 合成 → Whisper base 转写 → 比对源文本
> 环境：macOS ARM (Apple Silicon) + iOS 15.5 Simulator (x86_64)

### 9.1 Kokoro INT8 (macOS ARM 原生)

使用 `zm_yunxi` (男·中文, sid=50) 和 `zf_xiaobei` (女·中文, sid=45)：

| Voice | 输入文本 | STT 转写结果 | 结果 |
|-------|---------|-------------|------|
| zm_yunxi | 你好世界 | 你好世界 | ✅ |
| zm_yunxi | 今天天气不错 | 今天天气不错 | ✅ |
| zm_yunxi | 这是一个语音合成测试 | 這是一個語音合成測試 | ✅（繁简差异） |
| zm_yunxi | 第一章 从前有一座山 | 第一章,从前有一座山 | ✅（标点差异） |
| zm_yunxi | 他说这本书很有意思 | 他说这本书很优于死 | ❌ |
| zf_xiaobei | 你好世界 | 你好世界 | ✅ |
| zf_xiaobei | 今天天气不错 | 今天天气不错 | ✅ |
| zf_xiaobei | 这是一个语音合成测试 | 這是一個語音合常措施 | ❌ |
| zf_xiaobei | 第一章 从前有一座山 | 第一掌,从前有一座山 | ❌（章→掌） |
| zf_xiaobei | 他说这本书很有意思 | 他说这本书很有意思 | ✅ |
| af_alloy | Hello world, this is a test. | Hello world, this is a test. | ✅ |

**Kokoro 中文准确率**：zm_yunxi 4/5 (80%)，zf_xiaobei 3/5 (60%)

### 9.2 Piper Huayan (macOS ARM 原生)

| 输入文本 | STT 转写结果 | 结果 |
|---------|-------------|------|
| 你好世界 | 你好世間 | ❌（世界→世間） |
| 欢迎使用语音合成 | 換音使用語音合成 | ❌（欢迎→換音） |
| 今天天气不错 | 今天天气不错 | ✅ |
...
1. **Kokoro INT8 在 x86_64 iOS Simulator 上产生全 NaN 样本**（INT8 量化依赖 ARM NEON 指令）。原生 ARM 上完全正常。
2. **Kokoro > Piper** 中文质量明显更好（80% vs 33% STT 准确率）。
3. **Kokoro 必须使用中文 voice**（sid 45-52），用英文 voice（sid 0, af_alloy）合成中文效果极差。
4. **Piper Huayan 只支持中文**，输入英文会产生短促噪音。
5. Whisper base 模型本身有一定误差（繁简转换、同音字），实际 TTS 质量可能比 STT 准确率更高。

### 9.4 结论

| 模型 | 推荐场景 | 默认 Voice |
|------|---------|-----------|
| **Kokoro INT8** | App 端首选（多语言、高质量、126MB） | zm_yunxi (50) 中文 / af_alloy (0) 英文 |
| **Piper Huayan** | App 端中文备选（60MB、更快加载） | sid 0（单 speaker） |
| **Kokoro FP32** | 质量要求更高场景（333MB） | 同 INT8 |
