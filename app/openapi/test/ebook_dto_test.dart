import 'package:test/test.dart';
import 'package:openapi/openapi.dart';

// tests for EbookDto
void main() {
  final instance = EbookDtoBuilder();
  // TODO add properties to the builder and call build()

  group(EbookDto, () {
    // String id
    test('to test the property `id`', () async {
      // TODO
    });

    // int size
    test('to test the property `size`', () async {
      // TODO
    });

    // int ctime
    test('to test the property `ctime`', () async {
      // TODO
    });

    // int utime
    test('to test the property `utime`', () async {
      // TODO
    });

    // String title
    test('to test the property `title`', () async {
      // TODO
    });

    // String subTitle
    test('to test the property `subTitle`', () async {
      // TODO
    });

    // String language
    test('to test the property `language`', () async {
      // TODO
    });

    // String coverUrl
    test('to test the property `coverUrl`', () async {
      // TODO
    });

    // String uuid
    test('to test the property `uuid`', () async {
      // TODO
    });

    // String isbn
    test('to test the property `isbn`', () async {
      // TODO
    });

    // String asin
    test('to test the property `asin`', () async {
      // TODO
    });

    // String identifier
    test('to test the property `identifier`', () async {
      // TODO
    });

    // String category
    test('to test the property `category`', () async {
      // TODO
    });

    // String author
    test('to test the property `author`', () async {
      // TODO
    });

    // String authorUrl
    test('to test the property `authorUrl`', () async {
      // TODO
    });

    // String authorSort
    test('to test the property `authorSort`', () async {
      // TODO
    });

    // String publisher
    test('to test the property `publisher`', () async {
      // TODO
    });

    // String description
    test('to test the property `description`', () async {
      // TODO
    });

    // bool favStatus
    test('to test the property `favStatus`', () async {
      // TODO
    });

    // String pubdate
    test('to test the property `pubdate`', () async {
      // TODO
    });

    // num rating
    test('to test the property `rating`', () async {
      // TODO
    });

    // String publisherUrl
    test('to test the property `publisherUrl`', () async {
      // TODO
    });

    // int countVisit
    test('to test the property `countVisit`', () async {
      // TODO
    });

    // int countDownload
    test('to test the property `countDownload`', () async {
      // TODO
    });

    // num progress
    test('to test the property `progress`', () async {
      // TODO
    });

    // int progressIndex
    test('to test the property `progressIndex`', () async {
      // TODO
    });

    // int paraPosition
    test('to test the property `paraPosition`', () async {
      // TODO
    });

  });
}
